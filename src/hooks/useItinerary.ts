...
import { itineraryService, ItineraryResponse, ItineraryDay } from "@/services/itinerary";
import { mapsService, RouteResult } from "@/services/maps";
import { weatherService } from "@/services/weather";
import { cache } from "@/utils/cache";
import { withRetry, categorizeError, AppError, parallelFetch, createAbortController, isAbortedError } from "@/utils/errorHandler";
...
const TRAVEL_STYLE_KEYWORDS: Record<string, string[]> = {
  ...
  spiritual: ["temple", "spiritual", "pilgrimage", "ashram", "meditation", "yoga"],
...
function parseIntent(query: string): ParsedIntent {
  ...
  const fromMatch = lowerQuery.match(/from\s+([a-zA-Z\s]+?)(?:\s+to|\s+for|\s+in|\s*$)/);
  const origin = fromMatch ? fromMatch[1].trim() : "";
  
  // Extract destination (to X)
  const toMatch = lowerQuery.match(/to\s+([a-zA-Z\s]+?)(?:\s+for|\s+under|\s+in|\d|\s*$)/);
  const destination = toMatch ? toMatch[1].trim() : extractDestinationFallback(query);
...
export function useItinerary(options: UseItineraryOptions = {}): UseItineraryReturn {
  const {
    autoFetch = false,
    cacheEnabled = true,
    cacheTTL = CACHE_TTL,
  ...
  const cancel = useCallback(() => {
    ...
    setState(prev => ({ ...prev, loading: false, progress: { step: "cancelled", percentage: 0 } }));
  }, []);
  ...
  const generate = useCallback(async (query: string) => {
    ...
    if (cacheEnabled) {
      ...
      const localCached = await cache.get<{ itinerary: ItineraryResponse; route: RouteResult }>(
    ...
    try {
      ...
      const results = await parallelFetch({
        ...
        itinerary: async () => {
          ...
          return withRetry(
            () => itineraryService.generate(destination, days, 2, budget, query),
            { maxAttempts: maxRetries }
          );
        },
        weather: async () => {
      ...
      if (!itinerary) {
        throw new Error("Unable to generate itinerary. Please try a different destination.");
      }
