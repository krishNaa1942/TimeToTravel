...
const QuickStatCard = memo(({ icon, label, value, color = colors.primary, testID }: QuickStatCardProps) => (
  <View style={styles.quickStatCard} testID={testID} accessibilityRole="button" accessibilityLabel={`${label}: ${value}`}>
...
const HeroSection = memo(({ 
  destination, heroImage, isFavorite, onToggleFavorite, onBack, onShare, scrollY 
}: HeroSectionProps) => {
    ...
    <Animated.View style={[styles.heroContainer, { transform: [{ translateY: heroTranslateY }, { scale: heroScale }] }]}>
      {heroImage ? (
        ...
        <View style={[styles.heroImage, styles.heroPlaceholder]}>
          <MaterialCommunityIcons name="image-off" size={64} color={colors.gray} />
        </View>
      )}
...
const TagChips = memo(({ bestSeason, categories }: TagChipsProps) => {
  ...
  return (
    <View style={styles.chipsContainer} accessible={true} accessibilityLabel="Tags and categories">
      {bestSeason && (
        <View 
...
const HighlightText = memo(({ highlight }: HighlightTextProps) => {
  ...
  return (
    <View style={styles.highlightContainer} accessible={true} accessibilityLabel="Highlight">
...
const SectionHeader = memo(({ title, icon, actionLabel, onAction }: SectionHeaderProps) => (
...
const BudgetPreview = memo(({ destination, onExpand }: BudgetPreviewProps) => {
  const estimatedBudget = useMemo(() => {
    if (destination.priceRange) {
      return Math.round((destination.priceRange.min + destination.priceRange.max) / 2);
    }
    return 25000;
    ...
    >
      <View style={styles.budgetPreviewContent}>
        <View style={styles.budgetPreviewLeft}>
          <View style={[styles.budgetBadge, { backgroundColor: `${budgetLevel.color}15` }]}>
            <MaterialCommunityIcons name="wallet" size={20} color={budgetLevel.color} />
          </View>
          <View style={styles.budgetPreviewInfo}>
            <Text style={styles.budgetLabel}>Estimated Budget</Text>
            <Text style={styles.budgetValue}>{formatBudget(estimatedBudget)}</Text>
            <Text style={[styles.budgetLevelText, { color: budgetLevel.color }]}>
...
const PhotoGallery = memo(({ photos, onViewAll, onPhotoPress }: PhotoGalleryProps) => {
  const renderPhoto = useCallback(({ item, index }: { item: UnsplashImage; index: number }) => (
    <PressableScale 
    ...
    >
      <Image source={{ uri: item.url_small || item.url_regular }} style={styles.photo} resizeMode="cover" />
      <View style={styles.photoOverlay}>
        <MaterialCommunityIcons name="camera" size={12} color="#FFF" />
        <Text style={styles.photoCredit} numberOfLines={1}>{item.photographer}</Text>
      </View>
    </PressableScale>
  ), [onPhotoPress]);
...
const PhotoViewerModal = memo(({ visible, photos, initialIndex, onClose }: PhotoViewerModalProps) => {
  ...
  const renderPhoto = useCallback(({ item }: { item: UnsplashImage }) => (
        ...
        <View style={styles.thumbnailStrip}>
          <FlatList
            ...
            renderItem={({ item, index }) => (
              ...
              >
                <Image source={{ uri: item.url_thumb }} style={styles.thumbnailImage} />
              </TouchableOpacity>
            )}
          />
        </View>
...
const ActionBar = memo(({ onBudgetPress, onItineraryPress, onChatPress, onPackingPress }: ActionBarProps) => {
  ...
  return (
    <View style={styles.actionBarContainer}>
      <View style={styles.actionBarRow}>
        ...
        >
          <View style={[styles.actionBarIcon, { backgroundColor: `${colors.primary}15` }]}>
        ...
        >
          <View style={[styles.actionBarIcon, { backgroundColor: `${colors.secondary}15` }]}>
            <MaterialCommunityIcons name="map-marker-path" size={22} color={colors.secondary} />
          </View>
        ...
        >
          <View style={[styles.actionBarIcon, { backgroundColor: "rgba(16, 185, 129, 0.08)" }]}>
        ...
        >
          <View style={[styles.actionBarIcon, { backgroundColor: "#F59E0B15" }]}>
            <MaterialCommunityIcons name="bag-suitcase" size={22} color="#F59E0B" />
          </View>
...
const TravelTipCard = memo(({ tip, icon }: TravelTipCardProps) => (
  <View style={styles.tipCard} accessible={true} accessibilityLabel={`Tip: ${tip}`}>
...
const EmptyState = memo(({ title, message, icon }: EmptyStateProps) => (
  <View style={styles.emptyContainer} accessible={true} accessibilityLabel={`${title}: ${message}`}>
...
export default function DestinationDetailScreen() {
  ...
  const travelTips = useMemo(() => {
    ...
    if (destination.best_season) {
      tips.push({ tip: `Best time to visit: ${destination.best_season}`, icon: "calendar-check" });
    }
    
    if (categories.includes("beach")) {
      tips.push({ tip: "Don't forget sunscreen and beachwear!", icon: "umbrella" });
    }
    
    if (categories.includes("mountain") || categories.includes("hills")) {
      tips.push({ tip: "Pack warm clothes for cool evenings", icon: "weather-snowy" });
    }
    
    if (categories.includes("heritage") || categories.includes("religious")) {
      tips.push({ tip: "Dress modestly for temple visits", icon: "account-details" });
    }
    
    if (categories.includes("adventure")) {
      tips.push({ tip: "Book adventure activities in advance", icon: "hiking" });
    }
    
    // Generic tips
    tips.push({ tip: "Book accommodations early for best rates", icon: "bed" });
    tips.push({ tip: "Keep a copy of important documents", icon: "file-document" });
      ...
      >
        ...
        <View style={styles.quickStatsContainer}>
          <QuickStatCard 
            icon="thermometer" 
            label="Weather" 
            value={weather ? `${Math.round(weather.temperature_c || 0)}°C` : "--"} 
            color="#F59E0B"
            testID="weather-stat"
          />
          <QuickStatCard 
            ...
            color={safety?.overall_score && safety.overall_score >= 7 ? "#10B981" : "#F59E0B"}
            testID="safety-stat"
          />
          <QuickStatCard 
            icon="image-multiple" 
        ...
        <TagChips bestSeason={destination.best_season} categories={destination.category} />
        <HighlightText highlight={destination.highlight} />
        
        {/* Budget Preview */}
        <View style={styles.section}>
          <BudgetPreview destination={destination} onExpand={handleBudgetPress} />
        </View>
        ...
        {hasPartialError && (
          <View style={styles.partialErrorBanner} accessibilityRole="alert">
            ...
            <Text style={styles.partialErrorText}>Some information may be unavailable</Text>
          </View>
        )}
        ...
        {weather ? (
          <View style={styles.section}>
            <SectionHeader title="Current Weather" icon="weather-partly-cloudy" />
        ...
        {travelTips.length > 0 && (
          <View style={styles.section}>
            ...
            <View style={styles.tipsContainer} accessible={true} accessibilityLabel="Travel tips">
              {travelTips.map((tip, idx) => (
                <TravelTipCard key={`tip-${idx}`} tip={tip.tip} icon={tip.icon} />
              ))}
            </View>
          </View>
        )}
