...
// Components
import FeatureCard, { FeatureCardSkeleton } from '@/components/Trips/FeatureCard';
import { FeatureConfig, FeatureCategory, QuickAction } from '@/components/Trips/types';
...
const HeaderSection: React.FC = () => {
  ...
  const { greeting, emoji, streakMessage, levelMessage, xpProgress } = usePersonalizedGreeting();
    ...
    >
      ...
      {travelProgress && (
        <View style={styles.statsRow}>
          <StatItem emoji="🌍" value={travelProgress.countriesVisited} label="Countries" />
          <StatItem emoji="✈️" value={travelProgress.totalTrips} label="Trips" />
          <StatItem emoji="📍" value={travelProgress.citiesVisited} label="Cities" />
          <StatItem emoji="📅" value={travelProgress.upcomingTrips} label="Upcoming" />
        </View>
      )}
    </Animated.View>
  );
...
const QuickActionsSection: React.FC = () => {
  ...
  const handleAction = useCallback((type: string) => {
    ...
    switch (type) {
      ...
      case 'check_weather':
        ...
        navigation.dispatch(
          CommonActions.navigate({ name: 'ExploreTab' })
        );
        break;
      case 'add_expense':
        navigation.dispatch(
  ...
  const quickActions = [
    { id: '1', type: 'resume_trip' as const, title: 'Resume Trip', subtitle: 'Paris 2024', emoji: '✈️', urgency: 'high' as const },
    { id: '2', type: 'check_weather' as const, title: 'Weather', subtitle: 'Check forecast', emoji: '🌤️', urgency: 'low' as const },
    { id: '3', type: 'add_expense' as const, title: 'Add Expense', subtitle: 'Track spending', emoji: '💰', urgency: 'medium' as const },
    { id: '4', type: 'add_note' as const, title: 'Add Note', subtitle: 'Journal entry', emoji: '📝', urgency: 'low' as const },
  ];
...
const CategoryFilter: React.FC = () => {
  ...
  const handleSelect = useCallback((category: FeatureCategory | 'all') => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    setSelectedCategory(category);
  }, [setSelectedCategory]);

  const allCategory = { key: 'all', label: 'All', emoji: '✨', count: features.length };
    ...
    >
      <FlatList
        ...
        renderItem={({ item, index }) => {
          ...
          return (
            <Animated.View entering={FadeIn.delay(350 + index * 30).springify()}>
              <CategoryChip
                emoji={item.emoji}
                label={item.label}
                count={item.count}
                isSelected={isSelected}
                onPress={() => handleSelect(item.key as FeatureCategory | 'all')}
              />
            </Animated.View>
          );
        }}
...
const CategoryChip: React.FC<{
  ...
  <Pressable onPress={onPress} style={[styles.categoryChip, isSelected && styles.categoryChipSelected]}>
    <BlurView
...
const RecommendationsSection: React.FC = () => {
  ...
  const handlePress = useCallback((feature: FeatureConfig) => {
    trackFeatureClick(feature.id);
    if (feature.screen) {
      navigation.dispatch(
        CommonActions.navigate({ name: feature.screen as string, params: feature.params })
      );
    }
  ...
  return (
    ...
    >
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => (
          <Animated.View entering={SlideInRight.delay(450 + index * 50).springify()}>
            <RecommendationCard
              recommendation={item}
...
const RecommendationCard: React.FC<{
  ...
  <Pressable onPress={onPress} style={styles.recommendationCard}>
    ...
    <View style={styles.recommendationContent}>
      <Text style={styles.recommendationEmoji}>{recommendation.feature.emoji}</Text>
      <Text style={styles.recommendationTitle}>{recommendation.feature.title}</Text>
...
const FeatureGrid: React.FC = () => {
  const { features, isLoading, isFavorite, trackFeatureClick } = useTripsFeatures();
  ...
  const renderItem = useCallback(({ item, index }: { item: FeatureConfig; index: number }) => (
    <Animated.View
  ...
  return (
    <Animated.View entering={FadeIn.delay(500)} style={styles.featureGridSection}>
