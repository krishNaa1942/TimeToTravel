...
const PERSONALITY_PROFILES: Record<string, Partial<TravelPersonality>> = {
  explorer: {
    ...
    icon: '🗺️',
    color: '#4ECDC4',
    description: 'You thrive on discovering hidden gems and immersing yourself in local cultures.',
    strengths: ['Finding authentic experiences', 'Connecting with locals', 'Off-the-beaten-path discoveries'],
    tips: ['Try local homestays for authentic experiences', 'Learn basic local phrases'],
  },
  luxury: {
    ...
    strengths: ['Finding best hotels', 'Curating premium experiences', 'Attention to detail'],
    tips: ['Book during off-peak for better rates', 'Use loyalty programs strategically'],
  },
  budget: {
    ...
    icon: '💡',
    color: '#10B981',
    description: 'You have a knack for finding great deals and stretching your travel budget.',
    strengths: ['Finding deals', 'Budget optimization', 'Value-for-money experiences'],
    tips: ['Book flights 6-8 weeks in advance', 'Use price alerts for accommodations'],
  },
  adventurer: {
    ...
    icon: '🏔️',
    color: '#F59E0B',
    description: 'You crave adrenaline and seek out challenging outdoor experiences.',
    strengths: ['Finding adventure spots', 'Outdoor activities', 'Pushing boundaries'],
  ...
  foodie: {
    ...
    icon: '🍜',
    color: '#EF4444',
    description: 'Food is your gateway to understanding cultures and destinations.',
    strengths: ['Finding local cuisine', 'Food photography', 'Cooking classes'],
    tips: ['Research food festivals before traveling', 'Ask locals for hidden gems'],
  },
  relaxer: {
  ...
  social: {
    ...
    icon: '🦋',
    color: '#EC4899',
    description: 'Group travel and making connections defines your travel style.',
    strengths: ['Group coordination', 'Meeting new people', 'Shared experiences'],
...
const BADGE_DEFINITIONS: Omit<Badge, 'earnedAt' | 'progress'>[] = [
  // Exploration Badges
  { id: 'first_trip', name: 'First Steps', description: 'Completed your first trip', icon: '👶', category: 'exploration', rarity: 'common', target: 1, isUnlocked: false, xpReward: 50 },
  { id: 'explorer_5', name: 'Wanderer', description: 'Visited 5 destinations', icon: '🚶', category: 'exploration', rarity: 'common', target: 5, isUnlocked: false, xpReward: 100 },
  { id: 'explorer_10', name: 'Explorer', description: 'Visited 10 destinations', icon: '🧭', category: 'exploration', rarity: 'rare', target: 10, isUnlocked: false, xpReward: 250 },
  { id: 'explorer_25', name: 'Adventurer', description: 'Visited 25 destinations', icon: '🌏', category: 'exploration', rarity: 'epic', target: 25, isUnlocked: false, xpReward: 500 },
  { id: 'explorer_50', name: 'World Traveler', description: 'Visited 50 destinations', icon: '🌍', category: 'exploration', rarity: 'legendary', target: 50, isUnlocked: false, xpReward: 1000 },
  { id: 'globetrotter', name: 'Globetrotter', description: 'Visited 5 countries', icon: '🌎', category: 'exploration', rarity: 'epic', target: 5, isUnlocked: false, xpReward: 400 },
  
  // Planning Badges
  { id: 'planner_pro', name: 'Planner Pro', description: 'Created 10 trip plans', icon: '📋', category: 'planning', rarity: 'common', target: 10, isUnlocked: false, xpReward: 150 },
  { id: 'early_bird', name: 'Early Bird', description: 'Booked 5 trips in advance', icon: '🌅', category: 'planning', rarity: 'rare', target: 5, isUnlocked: false, xpReward: 200 },
  
  // Financial Badges
  { id: 'budget_master', name: 'Budget Master', description: 'Stayed under budget on 5 trips', icon: '💰', category: 'financial', rarity: 'rare', target: 5, isUnlocked: false, xpReward: 200 },
  { id: 'saver', name: 'Smart Saver', description: 'Saved ₹10,000 through deals', icon: '💎', category: 'financial', rarity: 'epic', target: 10000, isUnlocked: false, xpReward: 300 },
  
  // Social Badges
  { id: 'storyteller', name: 'Storyteller', description: 'Shared 10 travel stories', icon: '📖', category: 'social', rarity: 'common', target: 10, isUnlocked: false, xpReward: 100 },
  { id: 'photographer', name: 'Travel Photographer', description: 'Uploaded 100 photos', icon: '📸', category: 'social', rarity: 'rare', target: 100, isUnlocked: false, xpReward: 250 },
  { id: 'influencer', name: 'Travel Influencer', description: 'Got 100 likes on your content', icon: '⭐', category: 'social', rarity: 'epic', target: 100, isUnlocked: false, xpReward: 400 },
  
  // Special Badges
  { id: 'streak_7', name: 'Week Warrior', description: '7-day travel streak', icon: '🔥', category: 'special', rarity: 'rare', target: 7, isUnlocked: false, xpReward: 150 },
  { id: 'streak_30', name: 'Month Master', description: '30-day travel streak', icon: '🔥', category: 'special', rarity: 'epic', target: 30, isUnlocked: false, xpReward: 500 },
  { id: 'night_owl', name: 'Night Owl', description: 'Booked a trip after midnight', icon: '🦉', category: 'special', rarity: 'common', target: 1, isUnlocked: false, xpReward: 50 },
];
...
class TravelIntelligenceEngine {
  ...
  computeTravelDNA(stats: TravelStats, behaviorData?: any): TravelDNA {
    ...
    const totalSpending = Object.values(spendingBreakdown).reduce((a: number, b: any) => a + b, 0) as number;
    ...
    const explorerScore = Math.min(100, Math.round(
      (stats.destinations_visited * 10) + 
      (stats.places_visited * 2)
    ));

    // Luxury score: Based on hotel/premium spending
    const luxuryScore = Math.min(100, Math.round(
    ...
    // Foodie score
    const foodieScore = Math.min(100, Math.round(
      getSpendingPercent('food') * 1.5 +
      getSpendingPercent('restaurants') * 1.2
    ));

    // Adventure score
    const adventureScore = Math.min(100, Math.round(
    ...
    // Relax score
    const relaxScore = Math.min(100, Math.round(
    ...
    const socialScore = Math.min(100, Math.round(
      stats.favorites_count * 2 +
      stats.reservations.total * 5
    ));

    // Planner score
    const plannerScore = Math.min(100, Math.round(
      stats.trips.planning * 15 +
      stats.reservations.total * 5
    ));

    // Spontaneous score
    const spontaneousScore = Math.min(100, Math.round(
      100 - plannerScore * 0.5 +
      (stats.trips.active * 10)
    ));
  ...
  determinePersonality(dna: TravelDNA): TravelPersonality {
    ...
    const profile = PERSONALITY_PROFILES[dominant] || PERSONALITY_PROFILES.explorer;
  ...
  generateInsights(stats: TravelStats, dna: TravelDNA): AIInsight[] {
    ...
    if (stats.trips.total > 0) {
      ...
      if (ratio > 1.5) {
        ...
        insights.push({
          ...
          description: 'You have opportunities for more adventures. Consider planning your next getaway!',
    ...
    if (stats.total_spent > 0) {
      ...
      if (avgPerTrip < 5000) {
        ...
        insights.push({
          ...
          description: 'You prefer premium experiences. Consider loyalty programs for better value.',
          createdAt: now,
          isRead: false,
          isDismissed: false,
        });
    ...
    if (dna.foodie > 60) {
      insights.push({
        ...
        description: 'Your spending shows a love for food experiences. Try food tours on your next trip!',
        createdAt: now,
        isRead: false,
        isDismissed: false,
      });
    ...
    if (dna.adventure > 70) {
      insights.push({
        ...
        description: 'You crave adventure! Consider Rishikesh for rafting or Manali for trekking.',
  ...
  computeFinancialHealth(stats: TravelStats): FinancialHealth {
    ...
    else if (score >= 40) { grade = 'D'; status = 'needs-attention'; }
    ...
    if (categories.length > 0 && totalSpent > 0) {
      ...
      const topPercent = Math.round(((topCategory[1] as number) / totalSpent) * 100);
    ...
    return {
      ...
      status,
      insights,
      recommendations: this.getFinancialRecommendations(score, avgTripCost, categories),
  ...
  private getFinancialRecommendations(
    ...
    if (avgCost > 15000) {
      ...
      recommendations.push('Look for package deals to save on combined bookings');
    }
    ...
    if (hotelSpending && (hotelSpending[1] as number) > avgCost * 0.5) {
      recommendations.push('Try alternative accommodations like homestays or Airbnb');
    }
  ...
  computeSpendingCategories(stats: TravelStats): SpendingCategory[] {
    ...
    return Object.entries(spendingBreakdown).map(([name, amount]) => {
      ...
      return {
        ...
        trend: 'stable',
        trendValue: 0,
        status: percentage > 40 ? 'over' : percentage < 20 ? 'under' : 'on-track',
        insights: this.getCategoryInsights(name, amt, percentage),
      };
    });
  }

  private getCategoryInsights(category: string, amount: number, percentage: number): string[] {
    const insights: string[] = [];

    if (category === 'food' && percentage > 30) {
      insights.push('Consider trying local street food for authentic experiences');
  ...
  computeEnhancedStats(stats: TravelStats): EnhancedTravelStats {
    ...
    return {
      trips: {
        ...
        avgDuration: stats.trips.total > 0 ? Math.round(stats.total_travel_days / stats.trips.total) : 0,
      ...
      spending: {
        ...
        avgPerTrip: stats.trips.total > 0 ? Math.round(stats.total_spent / stats.trips.total) : 0,
        avgPerDay: stats.total_travel_days > 0 ? Math.round(stats.total_spent / stats.total_travel_days) : 0,
        currency: 'INR',
      },
      engagement: {
      ...
      time: {
        ...
        avgTripLength: stats.trips.total > 0 ? Math.round(stats.total_travel_days / stats.trips.total) : 0,
