...
import React, { memo, useMemo, useCallback, useState, createElement } from 'react';
import {
  View,
  Text,
  StyleSheet,
...
const StreamingCursor: React.FC<StreamingCursorProps> = memo(({ opacity, color }) => {
  return (
    <RNAnimated.Text
...
const getNodeStyle = (
  ...
  switch (node.type) {
    ...
    case 'link':
      return { ...baseStyle, color: theme.linkColor, textDecorationLine: 'underline' as const };
    default:
      return baseStyle;
  }
};
...
const RenderNode: React.FC<RendererProps & { depth?: number }> = memo(({
  ...
  switch (node.type) {
        ...
        <View style={[styles.codeBlock, { backgroundColor: theme.codeBackground }]}>
          {node.language && (
            <View style={styles.codeHeader}>
              ...
              >
                <MaterialCommunityIcons name="content-copy" size={14} color={theme.textMuted} />
              </TouchableOpacity>
            </View>
          )}
          <ScrollView
    ...
    case 'image':
      return (
        <TouchableOpacity
          ...
          accessibilityLabel={node.alt as string || 'Image'}
        >
      ...
      return (
        <View style={styles.list}>
          {node.children?.map((item, index) => (
            <View key={item.id || `${node.id}-${index}`} style={styles.listItem}>
    ...
    case 'hr':
      return <View style={[styles.hr, { backgroundColor: theme.borderColor }]} />;
...
const renderImageContent = (content: MessageContent, theme: MessageTextTheme) => {
...
const renderVideoContent = (content: MessageContent, theme: MessageTextTheme) => {
...
const renderLinkContent = (content: MessageContent, theme: MessageTextTheme) => {
  ...
  return (
    <View style={[styles.linkPreview, { borderColor: theme.borderColor }]}>
      {linkContent.preview?.image && (
        <Text style={styles.placeholderText}>🖼️ {linkContent.preview.title}</Text>
      )}
...
const renderAICardContent = (content: MessageContent, theme: MessageTextTheme) => {
...
const MessageText: React.FC<MessageTextProps> = memo(({
  ...
  const theme = useMemo(() => ({ ...DEFAULT_THEME, ...propTheme }), [propTheme]);

  // Get text content
  const text = useMemo(() => {
    if (content.type === 'text' || content.type === 'markdown') {
      return isStreaming && streamingContent ? streamingContent : (content as TextContent).text;
    }
    return '';
  ...
  const computedAccessibilityLabel = useMemo(() => {
    ...
    if (parsedContent?.plainText) {
      return isStreaming ? `Streaming: ${parsedContent.plainText}` : parsedContent.plainText;
    }
